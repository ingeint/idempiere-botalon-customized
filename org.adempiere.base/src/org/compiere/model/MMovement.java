...
public class MMovement extends X_M_Movement implements DocAction {
	...
	public MMovementLine[] getLines(boolean requery) {
		...
		List<MMovementLine> list = new Query(getCtx(), I_M_MovementLine.Table_Name, whereClause, get_TrxName())
				...
				.setOrderBy(MMovementLine.COLUMNNAME_Line + "," + MMovementLine.COLUMNNAME_M_MovementLine_ID).list();
	...
	public MMovementConfirm[] getConfirmations(boolean requery) {
		...
		List<MMovementConfirm> list = new Query(getCtx(), I_M_MovementConfirm.Table_Name, "M_Movement_ID=?",
	...
	public String prepareIt() {
		...
		// Validate mandatory ASI on lines - IDEMPIERE-1770 - ASI validation must be
		// moved to MMovement.prepareIt
		for (MMovementLine line : lines) {
			...
			if (line.getM_AttributeSetInstance_ID() == 0) {
				if (product != null && product.isASIMandatoryFor(null, true)) {
					if (product.getAttributeSet() != null
							&& !product.getAttributeSet().excludeTableEntry(MMovementLine.Table_ID, true)) { // outgoing
						BigDecimal qtyDiff = line.getMovementQty();
						// verify if the ASIs are captured on lineMA
						MMovementLineMA mas[] = MMovementLineMA.get(getCtx(), line.getM_MovementLine_ID(),
								get_TrxName());
			...
			if (line.getM_AttributeSetInstanceTo_ID() == 0) {
				if (product != null && product.isASIMandatoryFor(null, false)
						&& line.getM_AttributeSetInstanceTo_ID() == 0) {
					if (product.getAttributeSet() != null
							&& !product.getAttributeSet().excludeTableEntry(MMovementLine.Table_ID, false)) { // incoming
		...
		for (int i = 0; i < lines.length; i++) {
			...
			try {
				if (product != null && product.isStocked()) {
					...
					if (!isReversal()) {
						BigDecimal qtyOnLineMA = MMovementLineMA.getManualQty(line.getM_MovementLine_ID(),
								get_TrxName());
					...
					if (line.getM_AttributeSetInstance_ID() == 0) {
						...
						for (int j = 0; j < mas.length; j++) {
							...
							//
							// Update Storage
							if (!MStorageOnHand.add(getCtx(), line.getM_Locator_ID(), line.getM_Product_ID(),
									ma.getM_AttributeSetInstance_ID(), ma.getMovementQty().negate(),
									ma.getDateMaterialPolicy(), get_TrxName())) {
								String lastError = CLogger.retrieveErrorString("");
								m_processMsg = "Cannot correct Inventory OnHand (MA) [" + product.getValue() + "] - "
										+ lastError;
								return DocAction.STATUS_Invalid;
							}
							...
							// Update Storage
							if (!MStorageOnHand.add(getCtx(), line.getM_LocatorTo_ID(), line.getM_Product_ID(),
									M_AttributeSetInstanceTo_ID, ma.getMovementQty(), ma.getDateMaterialPolicy(),
									get_TrxName())) {
								String lastError = CLogger.retrieveErrorString("");
								m_processMsg = "Cannot correct Inventory OnHand (MA) [" + product.getValue() + "] - "
										+ lastError;
							...
							trxFrom = new MTransaction(getCtx(), line.getAD_Org_ID(),
									MTransaction.MOVEMENTTYPE_MovementFrom, line.getM_Locator_ID(),
					...
					if (trxFrom == null) {
						...
						if (line.getMovementQty().compareTo(Env.ZERO) > 0) {
							...
							storages = MStorageOnHand.getWarehouseOfDate(getCtx(), 0, line.getM_Product_ID(),
									line.getM_AttributeSetInstance_ID(), null,
									MClient.MMPOLICY_FiFo.equals(product.getMMPolicy()), false, line.getM_Locator_ID(),
									get_TrxName(), getMovementDate());
						} else {
							// Case of reversal
							storages = MStorageOnHand.getWarehouseOfDate(getCtx(), 0, line.getM_Product_ID(),
									line.getM_AttributeSetInstanceTo_ID(), null,
									MClient.MMPOLICY_FiFo.equals(product.getMMPolicy()), false,
									line.getM_LocatorTo_ID(), get_TrxName(), getMovementDate());
						}
						...
						if (!MStorageOnHand.add(getCtx(), line.getM_Locator_ID(), line.getM_Product_ID(),
								line.getM_AttributeSetInstance_ID(), line.getMovementQty().negate(), effDateMPolicy,
								get_TrxName())) {
							String lastError = CLogger.retrieveErrorString("");
							m_processMsg = "Cannot correct Inventory OnHand (MA) [" + product.getValue() + "] - "
									+ lastError;
						...
						if (dateMPolicy == null && line.getMovementQty().signum() > 0)
							effDateMPolicy = getMovementDate();
						if (!MStorageOnHand.add(getCtx(), line.getM_LocatorTo_ID(), line.getM_Product_ID(),
								line.getM_AttributeSetInstanceTo_ID(), line.getMovementQty(), effDateMPolicy,
								get_TrxName())) {
							String lastError = CLogger.retrieveErrorString("");
							m_processMsg = "Cannot correct Inventory OnHand (MA) [" + product.getValue() + "] - "
									+ lastError;
						...
						//
						trxFrom = new MTransaction(getCtx(), line.getAD_Org_ID(),
								MTransaction.MOVEMENTTYPE_MovementFrom, line.getM_Locator_ID(), line.getM_Product_ID(),
								line.getM_AttributeSetInstance_ID(), line.getMovementQty().negate(), getMovementDate(),
								get_TrxName());
						trxFrom.setM_MovementLine_ID(line.getM_MovementLine_ID());
						if (!trxFrom.save()) {
							m_processMsg = "Transaction From not inserted (MA) [" + product.getValue() + "] - ";
						...
						//
						MTransaction trxTo = new MTransaction(getCtx(), line.getAD_Org_ID(),
								MTransaction.MOVEMENTTYPE_MovementTo, line.getM_LocatorTo_ID(), line.getM_Product_ID(),
								line.getM_AttributeSetInstanceTo_ID(), line.getMovementQty(), getMovementDate(),
								get_TrxName());
						trxTo.setM_MovementLine_ID(line.getM_MovementLine_ID());
						if (!trxTo.save()) {
	...
	protected void checkMaterialPolicy(MMovementLine line, BigDecimal qtyToDeliver) {
		...
		if (line.getM_AttributeSetInstance_ID() == 0) {
			...
			MStorageOnHand[] storages = MStorageOnHand.getWarehouseOfDate(getCtx(), 0, line.getM_Product_ID(), 0, null,
			...
			for (MStorageOnHand storage : storages) {
				if (storage.getQtyOnHand().compareTo(qtyToDeliver) >= 0) {
					MMovementLineMA ma = new MMovementLineMA(line, storage.getM_AttributeSetInstance_ID(), qtyToDeliver,
							storage.getDateMaterialPolicy(), true);
					ma.saveEx();
					qtyToDeliver = Env.ZERO;
					if (log.isLoggable(Level.FINE))
						log.fine(ma + ", QtyToDeliver=" + qtyToDeliver);
				} else {
					MMovementLineMA ma = new MMovementLineMA(line, storage.getM_AttributeSetInstance_ID(),
	...
	protected MMovement reverse(boolean accrual) {
		...
		for (int i = 0; i < oLines.length; i++) {
			...
			if (!rLine.save()) {
				m_processMsg = "Could not create Movement Reversal Line for @Line@ " + rLine.getLine()
						+ ", @M_Product_ID@=" + rLine.getProduct().getValue();
				return null;
			}

			// We need to copy MA
			if (rLine.getM_AttributeSetInstance_ID() == 0) {
				MMovementLineMA mas[] = MMovementLineMA.get(getCtx(), oLine.getM_MovementLine_ID(), get_TrxName());
				for (int j = 0; j < mas.length; j++) {
					MMovementLineMA ma = new MMovementLineMA(rLine, mas[j].getM_AttributeSetInstance_ID(),
	...
	public String getSummary() {
		...
		sb.append(": ").append(Msg.translate(getCtx(), "ApprovalAmt")).append("=").append(getApprovalAmt())
