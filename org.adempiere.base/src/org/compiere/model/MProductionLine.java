...
public class MProductionLine extends X_M_ProductionLine {
	...
	public String createTransactions(Timestamp date, boolean mustBeStocked) {
		...
		MStorageOnHand[] storages = MStorageOnHand.getWarehouseOfDate(getCtx(), 0,
				getM_Product_ID(), getM_AttributeSetInstance_ID(), null,
				MClient.MMPOLICY_FiFo.equals(prod.getMMPolicy()), false,
				getM_Locator_ID(), get_TrxName(), date);
		...
		else if (qtyToMove.signum() < 0 )
		{
			...
			if (asi.get_ID() == 0 && MAcctSchema.COSTINGLEVEL_BatchLot.equals(prod.getCostingLevel(acctSchema)) )
			{
				...
				String sqlWhere = "M_Product_ID=? AND M_Locator_ID=? AND M_AttributeSetInstance_ID > 0 AND DateMaterialPolicy <= ? ";
				MStorageOnHand storage = new Query(getCtx(), MStorageOnHand.Table_Name, sqlWhere, get_TrxName())
						.setParameters(getM_Product_ID(), getM_Locator_ID(), date)
