	...
	{		
		...
		for (int i = 0; i < oLines.length; i++)
		{
			...
			if (product != null && product.get_ID() != 0 && product.isStocked())
			{
				...
				storages = MStorageOnHand.getWarehouseOfDate(order.getCtx(), order.getM_Warehouse_ID(),
					oLines[i].getM_Product_ID(), oLines[i].getM_AttributeSetInstance_ID(),
					minGuaranteeDate, MClient.MMPOLICY_FiFo.equals(MMPolicy), true, 0, trxName, movementDate);
			} else {
				continue;
			}
			...
			try
			{
					...
					&& product.isStocked() )
				{
					...
					if (mtrx == null)
					{
						...
						{
							String status = moveOnHandToShipmentASI(product, sLine.getM_Locator_ID(), sLine.getM_AttributeSetInstance_ID(), Qty.negate(), getMovementDate(), sLine.get_ID(), false, get_TrxName());
							if (status != null)
								return status;
						}
						...
						if (pendingQty.signum() < 0) {  // taking from inventory
							MStorageOnHand[] storages = MStorageOnHand.getWarehouseOfDate(getCtx(), 0,
									sLine.getM_Product_ID(), sLine.getM_AttributeSetInstance_ID(), null,
									MClient.MMPOLICY_FiFo.equals(product.getMMPolicy()), false,
									sLine.getM_Locator_ID(), get_TrxName(), getMovementDate());
	...
	protected void checkMaterialPolicy(MInOutLine line,BigDecimal qty)
	{
		...
		if (product != null && line.getM_AttributeSetInstance_ID() == 0)
		{
			...
			else if(getMovementType().compareTo(MInOut.MOVEMENTTYPE_VendorReturns) == 0 || getMovementType().compareTo(MInOut.MOVEMENTTYPE_CustomerShipment) == 0)
			{
				...
				MStorageOnHand[] storages = MStorageOnHand.getWarehouseOfDate(getCtx(), getM_Warehouse_ID(), line.getM_Product_ID(), line.getM_AttributeSetInstance_ID(),
						minGuaranteeDate, MClient.MMPOLICY_FiFo.equals(MMPolicy), true, line.getM_Locator_ID(), get_TrxName(), false, getMovementDate());
