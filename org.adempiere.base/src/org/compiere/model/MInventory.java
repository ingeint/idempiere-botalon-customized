			...
			try
			{
						...
						&& product.isStocked() )
				{
					...
					if (mtrx == null)
					{
						...
						if (line.getM_AttributeSetInstance_ID() > 0)
						{
							Timestamp t = MStorageOnHand.getDateMaterialPolicyOfDate(line.getM_Product_ID(), line.getM_AttributeSetInstance_ID(), line.getM_Locator_ID(), line.get_TrxName(), getMovementDate());
							if (t != null)
								dateMPolicy = t;
							else 
	...
	{	
		...
		if (line.getM_AttributeSetInstance_ID() == 0)
		{
			...
			if (qtyDiff.signum() > 0)	//	Incoming Trx
			{
				...
				MStorageOnHand[] storages = MStorageOnHand.getWarehouseNegativeOfDate(getCtx(), getM_Warehouse_ID(), line.getM_Product_ID(), 0,
						null, MClient.MMPOLICY_FiFo.equals(product.getMMPolicy()), line.getM_Locator_ID(), get_TrxName(), false, getMovementDate());
				...
				if(qtyDiff.compareTo(Env.ZERO)>0)
				{
					...
					if (as != null && as.isInstanceAttribute())
					{
						...
						storages = MStorageOnHand.getWarehouseOfDate(getCtx(), getM_Warehouse_ID(), line.getM_Product_ID(), 0, null,
								false, true, 0, get_TrxName(), getMovementDate());
						for (MStorageOnHand storage : storages)
						{
					...
					if(qtyDiff.compareTo(Env.ZERO)>0)
					{
						...
						{
							String sqlWhere = "M_Product_ID=? AND M_Locator_ID=? AND QtyOnHand = 0 AND M_AttributeSetInstance_ID > 0 AND DateMaterialPolicy = ?  ";
							MStorageOnHand storage = new Query(getCtx(), MStorageOnHand.Table_Name, sqlWhere, get_TrxName())
									.setParameters(line.getM_Product_ID(), line.getM_Locator_ID(), getMovementDate())
			...
			else	//	Outgoing Trx
			{
				...
				MStorageOnHand[] storages = MStorageOnHand.getWarehouseOfDate(getCtx(), getM_Warehouse_ID(), line.getM_Product_ID(), 0,
						null, MClient.MMPOLICY_FiFo.equals(MMPolicy), true, line.getM_Locator_ID(), get_TrxName(), false, getMovementDate());
