	...
	implements DocAction
{
	...
	public String completeIt() {
		...
		MDepreciationWorkfile assetwk = MDepreciationWorkfile.get(getCtx(), getA_Asset_ID(), getPostingType(),
				get_TrxName());
		if (assetwk == null) {
			for (MAssetGroupAcct assetgrpacct : MAssetGroupAcct.forA_Asset_Group_ID(getCtx(),
					asset.getA_Asset_Group_ID(), getPostingType())) {
				if (A_SOURCETYPE_Imported.equals(getA_SourceType())
						&& assetgrpacct.getC_AcctSchema_ID() != getI_FixedAsset().getC_AcctSchema_ID())
					continue;
		...
		for (MDepreciationWorkfile assetworkFile : MDepreciationWorkfile.forA_Asset_ID(getCtx(), getA_Asset_ID(),
				get_TrxName())) {
			if (A_SOURCETYPE_Imported.equals(getA_SourceType())
					&& assetworkFile.getC_AcctSchema_ID() != getI_FixedAsset().getC_AcctSchema_ID())
				continue;
			...
			if (A_SOURCETYPE_Imported.equals(getA_SourceType())) {
				...
				if (assetworkFile.getC_AcctSchema().getC_Currency_ID() != getC_Currency_ID()) {
					BigDecimal convertedAssetCost = Env.ZERO;
					if (get_ValueAsBoolean("IsOverrideCurrencyRate")) {
						convertedAssetCost = getAssetValueAmt();
					} else {
						convertedAssetCost = MConversionRate.convert(getCtx(), getAssetSourceAmt(), getC_Currency_ID(),
								assetworkFile.getC_AcctSchema().getC_Currency_ID(), getDateAcct(),
								getC_ConversionType_ID(), getAD_Client_ID(), getAD_Org_ID());
					}
					assetworkFile.adjustCost(convertedAssetCost, getA_QTY_Current(), isA_CreateAsset()); // reset if
																											// isA_CreateAsset
				} else {
					assetworkFile.adjustCost(getAssetSourceAmt(), getA_QTY_Current(), isA_CreateAsset()); // reset if
																											// isA_CreateAsset
				}
			}
			...
			if (this.getA_CapvsExp().equals(A_CAPVSEXP_Capital)) {
				...
				MDepreciationExp.checkExistsNotProcessedEntries(assetworkFile.getCtx(), assetworkFile.getA_Asset_ID(),
						getDateAcct(), assetworkFile.getPostingType(), assetworkFile.get_TrxName());
				//
