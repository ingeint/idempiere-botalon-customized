 ...
 * @see https://sourceforge.net/p/adempiere/feature-requests/631/ Modifications:
 *      Added RMA functionality (Ashley Ramdass) Modifications: Generate DocNo^
 *      instead of using a new number whan an invoice is reversed (Diego
 *      Ruiz-globalqss)
 */
public class MInvoice extends X_C_Invoice implements DocAction, IDocsPostProcess {
	 ...
	 * @return Invoice
	 */
	public static MInvoice copyFrom(MInvoice from, Timestamp dateDoc, Timestamp dateAcct, int C_DocTypeTarget_ID,
			boolean isSOTrx, boolean counter, String trxName, boolean setOrder) {
		return copyFrom(from, dateDoc, dateAcct, C_DocTypeTarget_ID, isSOTrx, counter, trxName, setOrder, null);
	}
	 ...
	 */
	public static MInvoice copyFrom(MInvoice from, Timestamp dateDoc, Timestamp dateAcct, int C_DocTypeTarget_ID,
			boolean isSOTrx, boolean counter, String trxName, boolean setOrder, String documentNo) {
	 ...
	 * @return Invoice
	 */
	public static MInvoice copyFrom(MInvoice from, Timestamp dateDoc, int C_DocTypeTarget_ID, boolean isSOTrx,
			boolean counter, String trxName, boolean setOrder) {
		MInvoice to = copyFrom(from, dateDoc, dateDoc, C_DocTypeTarget_ID, isSOTrx, counter, trxName, setOrder);
		return to;
	} // copyFrom
	...
	public MInvoice(Properties ctx, MInvoice copy, String trxName) {
		...
		this.m_lines = copy.m_lines != null ? Arrays.stream(copy.m_lines).map(e -> {
			var v = new MInvoiceLine(ctx, e, trxName);
			v.m_parent = this;
			return v;
		}).toArray(MInvoiceLine[]::new) : null;
		this.m_taxes = copy.m_taxes != null ? Arrays.stream(copy.m_taxes).map(e -> {
			return new MInvoiceTax(ctx, e, trxName);
		}).toArray(MInvoiceTax[]::new) : null;
	}
	...
	public void setC_DocTypeTarget_ID(String DocBaseType) {
		...
		if (C_DocType_ID <= 0)
			log.log(Level.SEVERE, "Not found for AD_Client_ID=" + getAD_Client_ID() + " - " + DocBaseType);
		else {
	...
	private MInvoiceLine[] getLines(String whereClause) {
		...
		List<MInvoiceLine> list = new Query(getCtx(), I_C_InvoiceLine.Table_Name, whereClauseFinal, get_TrxName())
				.setParameters(getC_Invoice_ID()).setOrderBy("Line, C_InvoiceLine_ID").list();
	...
	public void setProcessed(boolean processed) {
		...
		if (get_ID() == 0)
			return;
		StringBuilder set = new StringBuilder("SET Processed='").append((processed ? "Y" : "N"))
	...
	public boolean validatePaySchedule() {
		MInvoicePaySchedule[] schedule = MInvoicePaySchedule.getInvoicePaySchedule(getCtx(), getC_Invoice_ID(), 0,
				get_TrxName());
		if (log.isLoggable(Level.FINE))
	...
	protected boolean beforeSave(boolean newRecord) {
		...
		if (!newRecord && (is_ValueChanged(COLUMNNAME_M_PriceList_ID) || is_ValueChanged(COLUMNNAME_DateInvoiced))) {
			int cnt = DB.getSQLValueEx(get_TrxName(),
					"SELECT COUNT(*) FROM C_InvoiceLine WHERE C_Invoice_ID=? AND M_Product_ID>0", getC_Invoice_ID());
			if (cnt > 0) {
				...
				if (is_ValueChanged(COLUMNNAME_DateInvoiced)) {
					...
					MPriceListVersion plOld = pList
							.getPriceListVersion((Timestamp) get_ValueOld(COLUMNNAME_DateInvoiced));
	...
	public String toString() {
		StringBuilder sb = new StringBuilder("MInvoice[").append(get_ID()).append("-").append(getDocumentNo())
	...
	protected boolean afterSave(boolean newRecord, boolean success) {
		...
		if (is_ValueChanged("AD_Org_ID")) {
			StringBuilder sql = new StringBuilder("UPDATE C_InvoiceLine ol").append(" SET AD_Org_ID =")
					.append("(SELECT AD_Org_ID").append(" FROM C_Invoice o WHERE ol.C_Invoice_ID=o.C_Invoice_ID) ")
					.append("WHERE C_Invoice_ID=").append(getC_Invoice_ID());
	...
	public BigDecimal getAllocatedAmt() {
		...
		String sql = "SELECT SUM(currencyConvert(al.Amount+al.DiscountAmt+al.WriteOffAmt,"
				...
				+ " INNER JOIN C_Invoice i ON (al.C_Invoice_ID=i.C_Invoice_ID) " + "WHERE al.C_Invoice_ID=?"
	...
	public BigDecimal getOpenAmt(Timestamp paymentDate) {
		BigDecimal retValue;
		if (paymentDate == null) {
			retValue = DB.getSQLValueBDEx(get_TrxName(), "SELECT invoiceOpen(?,?) FROM DUAL", getC_Invoice_ID(), 0);
		} else {
			retValue = DB.getSQLValueBDEx(get_TrxName(), "SELECT invoiceOpenToDate(?,?,?) FROM DUAL", getC_Invoice_ID(),
					0, paymentDate);
		}
		return retValue;
	} // getOpenAmt
	...
	public BigDecimal getDiscountAmt(Timestamp paymentDate) {
		BigDecimal retValue = DB.getSQLValueBDEx(get_TrxName(), "SELECT invoiceDiscount(?,?,?) FROM DUAL",
	...
	public String prepareIt() {
		...
		if (columnExists("DateAcctCustom"))
			if (get_Value("DateAcctCustom") != null)
				DateAcct = (Timestamp) get_Value("DateAcctCustom") ;

		MPeriod.testPeriodOpen(getCtx(), DateAcct, getC_DocTypeTarget_ID(), getAD_Org_ID());
		...
		if (getGrandTotal().signum() != 0 && (PAYMENTRULE_OnCredit.equals(getPaymentRule())
		...
		if (isSOTrx()) {
			...
			if ((doc.getDocBaseType().equals(MDocType.DOCBASETYPE_ARCreditMemo) && getGrandTotal().signum() < 0)
				...
				if (MBPartner.SOCREDITSTATUS_CreditStop.equals(bp.getSOCreditStatus())) {
					m_processMsg = "@BPartnerCreditStop@ - @TotalOpenBalance@=" + bp.getTotalOpenBalance()
	...
	public boolean calculateTaxTotal() {
		log.fine("");
		// Delete Taxes
		StringBuilder msgdb = new StringBuilder("DELETE FROM C_InvoiceTax WHERE C_Invoice_ID=")
	...
	public String completeIt() {
		...
		if (PAYMENTRULE_Cash.equals(getPaymentRule()) && !fromPOS) {
			...
			MBankAccount ba = new Query(getCtx(), MBankAccount.Table_Name, whereClause, get_TrxName())
					.setParameters(getAD_Org_ID(), getC_Currency_ID()).setOnlyActiveRecords(true)
		...
		for (int i = 0; i < lines.length; i++) {
			...
			if (!isSOTrx() && line.getM_InOutLine_ID() != 0 && line.getM_Product_ID() != 0 && !isReversal()) {
				...
				if (receipt.isProcessed()) {
					...
					BigDecimal movementQty = receiptLine.getM_InOut().getMovementType().charAt(1) == '-'
							? receiptLine.getMovementQty().negate()
			...
			if (line.getC_OrderLine_ID() != 0) {
				if (isSOTrx() || line.getM_Product_ID() == 0) {
					...
					if (line.getQtyInvoiced() != null) {
						ol.setQtyInvoiced(ol.getQtyInvoiced()
								.add(isCreditMemo() ? line.getQtyInvoiced().negate() : line.getQtyInvoiced()));
					}
				...
				// Order Invoiced Qty updated via Matching Inv-PO
					...
					if (po != null) {
						...
						MMatchInv[] matchInvoices = MMatchInv.getInvoiceLine(getCtx(), line.getC_InvoiceLine_ID(),
								get_TrxName());
						if (matchInvoices != null && matchInvoices.length > 0) {
							for (MMatchInv matchInvoice : matchInvoices) {
								...
								if (matchInvoice.getRef_MatchInv_ID() > 0) {
									MMatchInv refMatchInv = new MMatchInv(getCtx(), matchInvoice.getRef_MatchInv_ID(),
											get_TrxName());
		...
		if (invAmt == null) {
			m_processMsg = MConversionRateUtil.getErrorMessage(getCtx(), "ErrorConvertingCurrencyToBaseCurrency",
					getC_Currency_ID(), MClient.get(getCtx()).getC_Currency_ID(), getC_ConversionType_ID(),
		...
		if (isSOTrx()) {
			...
			//
			if (log.isLoggable(Level.FINE))
				log.fine("GrandTotal=" + getGrandTotal(true) + "(" + invAmt + ") BP Life=" + bp.getActualLifeTimeValue()
						+ "->" + newLifeAmt + ", Credit=" + bp.getSO_CreditUsed() + "->" + newCreditAmt + ", Balance="
			...
			if (log.isLoggable(Level.FINE))
				log.fine("GrandTotal=" + getGrandTotal(true) + "(" + invAmt + ") Balance=" + bp.getTotalOpenBalance()
						+ " -> " + newBalance);
		}
		...
		if (getAD_User_ID() != 0) {
			...
			StringBuilder msgset = new StringBuilder().append(Msg.translate(getCtx(), "C_Invoice_ID")).append(": ")
		...
		if (isSOTrx() && getC_Project_ID() != 0) {
			...
			if (C_CurrencyTo_ID != getC_Currency_ID())
				amt = MConversionRate.convert(getCtx(), amt, getC_Currency_ID(), C_CurrencyTo_ID, getDateAcct(), 0,
		...
		if (isSOTrx() && !isReversal()) {
			...
			int[] ids = MPaymentTransaction.getAuthorizationPaymentTransactionIDs(orderIDList, getC_Invoice_ID(),
					get_TrxName());
			if (ids.length > 0) {
				...
				for (int id : ids) {
					...
					if (!pt.setPaymentProcessor()) {
						if (pt.getC_PaymentProcessor_ID() > 0) {
							MPaymentProcessor pp = new MPaymentProcessor(getCtx(), pt.getC_PaymentProcessor_ID(),
									get_TrxName());
					...
					boolean isCIM = pt.getC_PaymentProcessor_ID() > 0 && pt.getCustomerPaymentProfileID() != null
				...
				// automatically void authorization payment and create a new sales payment when
				// invoiced amount is NOT equals to the authorized amount (applied to CIM
				// payment processor)
				if (getGrandTotal().compareTo(totalPayAmt) != 0 && ptList.size() > 0 && pureCIM) {
					// create a new sales payment
					MPaymentTransaction newSalesPT = MPaymentTransaction.copyFrom(ptList.get(0),
							new Timestamp(System.currentTimeMillis()), MPayment.TRXTYPE_Sales, "", get_TrxName());
					...
					for (MPaymentTransaction pt : ptList) {
						...
						pt.saveEx();
						if (!ok) {
							m_processMsg = Msg.getMsg(getCtx(), "VoidAuthorizationPaymentFailed") + ": "
									+ pt.getErrorMessage();
							return DocAction.STATUS_Invalid;
						}
					}
					...
					newSalesPT.saveEx();
					if (!ok) {
						m_processMsg = Msg.getMsg(getCtx(), "CreateNewSalesPaymentFailed") + ": "
	...
	private void setDefiniteDocumentNo() {
		if (isReversal()
				&& !MSysConfig.getBooleanValue(MSysConfig.Invoice_ReverseUseNewNumber, true, getAD_Client_ID())) // IDEMPIERE-1771
			return;
	...
	private MInvoice createCounterDoc() {
		...
		// Deep Copy
		MInvoice counter = copyFrom(this, getDateInvoiced(), getDateAcct(), C_DocTypeTarget_ID, !isSOTrx(), true,
				get_TrxName(), true);
		//
			...
			if (counterDT.getDocAction() != null) {
				...
				if (!counter.processIt(counterDT.getDocAction()))
					throw new AdempiereException(
							Msg.getMsg(getCtx(), "FailedProcessingDocument") + " - " + counter.getProcessMsg());
				// end added
				counter.saveEx(get_TrxName());
			}
		}
		...
		if (DOCSTATUS_Drafted.equals(getDocStatus()) || DOCSTATUS_Invalid.equals(getDocStatus())
			...
			for (int i = 0; i < lines.length; i++) {
				...
				if (old.compareTo(Env.ZERO) != 0) {
					...
					StringBuilder msgadd = new StringBuilder(Msg.getMsg(getCtx(), "Voided")).append(" (").append(old)
							.append(")");
	...
	private MInvoice reverse(boolean accrual) {
		...
		if (MSysConfig.getBooleanValue(MSysConfig.Invoice_ReverseUseNewNumber, true, getAD_Client_ID()))
			reversal = copyFrom(this, reversalDateInvoiced, reversalDate, getC_DocType_ID(), isSOTrx(), false,
					get_TrxName(), true);
		else
			reversal = copyFrom(this, reversalDateInvoiced, reversalDate, getC_DocType_ID(), isSOTrx(), false,
					get_TrxName(), true, getDocumentNo() + "^");
		if (reversal == null) {
		...
		// Create Allocation
		StringBuilder msgall = new StringBuilder().append(Msg.translate(getCtx(), "C_Invoice_ID")).append(": ")
				.append(getDocumentNo()).append("/").append(reversal.getDocumentNo());
		MAllocationHdr alloc = new MAllocationHdr(getCtx(), false, reversalDate, getC_Currency_ID(), msgall.toString(),
				get_TrxName());
		...
		if (!alloc.processIt(DocAction.ACTION_Complete))
			throw new AdempiereException(
					Msg.getMsg(getCtx(), "FailedProcessingDocument") + " - " + alloc.getProcessMsg());
		// end added
		alloc.saveEx();
	...
	public String getSummary() {
		...
		sb.append(": ").append(Msg.translate(getCtx(), "GrandTotal")).append("=").append(getGrandTotal()).append(" (#")
