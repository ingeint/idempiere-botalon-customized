...
public class MAssetDisposed extends X_A_Asset_Disposed implements DocAction {
	...
	private MAssetDisposed(MInvoiceLine invLine) {
		...
		if (invLine.getC_Invoice().getDocStatus().equals(MInvoice.DOCSTATUS_InProgress)) {
	...
	public String prepareIt() {
		...
		MDepreciationExp.checkExistsNotProcessedEntries(getCtx(), getA_Asset_ID(), getDateAcct(), getPostingType(),
				get_TrxName());
	...
	public String completeIt() {
		...
		if (!isDisposal()) {
			...
			if (method.equals(A_ACTIVATION_METHOD_Activation)) { // reactivation
		...
		// Preservation/Partial Retirement/etc
			...
			} else if (A_DISPOSED_METHOD_Simple.equals(method) || A_DISPOSED_METHOD_Trade.equals(method)) {
	...
	public String getSummary() {
		return new StringBuilder().append(getDocumentNo()).append("/").append(getDateDoc()).toString();
	}
	...
	protected boolean beforeSave(boolean newRecord) {
		...
		/*
		 * commented by @win - asset type if (!MAssetType.isFixedAsset(getA_Asset_ID()))
	...
	public static void updateFromAsset(I_A_Asset_Disposed bean) {
		...
		SetGetUtil.copyValues(SetGetUtil.wrap(bean), MAsset.Table_Name, asset_id,
				new String[] { MAsset.COLUMNNAME_IsDisposed, MAsset.COLUMNNAME_A_Asset_Status, "AD_Org_ID", });
	 ...
	 * @param bean
	 */
	public static void updateFromAsset(MAssetDisposed bean) {
		int asset_id = bean.getA_Asset_ID();
		MDepreciationWorkfile wk = MDepreciationWorkfile.get(Env.getCtx(), asset_id, bean.getPostingType(), null);
		if (wk != null) {
			bean.setA_Asset_Cost(wk.getA_Asset_Cost());
	...
	public static void setA_Disposal_Amt(I_A_Asset_Disposed bean) {
		...
		BigDecimal A_Accumulated_Depr_Delta = A_Accumulated_Depr.multiply(coef).setScale(precision,
	...
	public static void setA_Disposal_Amt(MAssetDisposed bean) {
	...
	private void createDisposal() {
		for (MDepreciationWorkfile assetwk : MDepreciationWorkfile.forA_Asset_ID(getCtx(), getA_Asset_ID(),
				get_TrxName())) {
			...
			change.setTextDetails(
					MRefList.getListDescription(getCtx(), "A_Update_Type", MAssetChange.CHANGETYPE_Disposal));
		...
		List<MDepreciationExp> list = MDepreciationExp.getNotProcessedEntries(getCtx(), getA_Asset_ID(),
				getPostingType(), get_TrxName());
