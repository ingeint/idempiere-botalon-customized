...
public class MProduct extends X_M_Product implements ImmutablePOSupport
{
	...
	public String getCostingLevel(MAcctSchema as)
	{
		 ...
		 X_M_Product_Acct productAcct = new Query(getCtx(), I_M_Product_Acct.Table_Name, "M_Product_ID=? and C_AcctSchema_ID=?", get_TrxName())
                .setParameters(get_ID(), as.getC_AcctSchema_ID())
                .first();		

		String costingLevelProduct = productAcct.get_ValueAsString("CostingLevel");
		
		if (!Strings.isNullOrEmpty(costingLevelProduct))
			return costingLevelProduct;
	...
	public String getCostingMethod(MAcctSchema as)
	{
		...
		String costingLevelProduct = productAcct.get_ValueAsString("CostingMethod");
