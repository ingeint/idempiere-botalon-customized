...
public abstract class Doc {
	/**************************************************************************
	 * Document Types -------------- C_DocType.DocBaseType and AD_Reference_ID=183
	 * C_Invoice: ARI, ARC, ARF, API, APC C_Payment: ARP, APP C_Order: SOO, POO
	 * M_Transaction: MMI, MMM, MMS, MMR C_BankStatement: CMB C_Cash: CMC
	 * C_Allocation: CMA GL_Journal: GLJ C_ProjectIssue PJI M_Requisition POR
	 **************************************************************************/
	 ...
	 */
	public static String postImmediate(MAcctSchema[] ass, int AD_Table_ID, int Record_ID, boolean force,
			String trxName) {
		...
		try {
			...
			if (AD_Table_ID == MInOut.Table_ID) {
				...
				for (MMatchPO matchPo : matchPos) {
					if (!matchPo.isPosted()) {
						error = postImmediate(ass, matchPo.get_Table_ID(), matchPo.get_ID(), force,
								matchPo.get_TrxName());
				...
				if (AD_Table_ID == MInvoice.Table_ID) {
					...
					for (MMatchInv matchInv : matchInvs) {
						if (!matchInv.isPosted()) {
							error = postImmediate(ass, matchInv.get_Table_ID(), matchInv.get_ID(), force,
	...
	/*
	 * If the transaction must be managed locally (false if it's managed externally
	 * by the caller)
	 */
	...
	public Doc(MAcctSchema as, Class<?> clazz, ResultSet rs, String defaultDocumentType, String trxName) {
		...
		p_po.load(p_po.get_TrxName()); // reload the PO to get any virtual column that was not obtained using the rs
										// (IDEMPIERE-775)
	...
	public final String post(boolean force, boolean repost) {
		...
		else if (m_DocStatus.equals(DocumentEngine.STATUS_Completed) || m_DocStatus.equals(DocumentEngine.STATUS_Closed)
				|| m_DocStatus.equals(DocumentEngine.STATUS_Voided)
				|| m_DocStatus.equals(DocumentEngine.STATUS_Reversed))
			;
		else {
			StringBuilder msgreturn = new StringBuilder("Invalid DocStatus='").append(m_DocStatus)
					.append("' for DocumentNo=").append(getDocumentNo());
		...
		sql.append(get_TableName()).append(" SET Processing='Y' WHERE ").append(get_TableName()).append("_ID=")
				.append(get_ID()).append(" AND Processed='Y' AND IsActive='Y'");
		if (!force)
		...
		if (DB.executeUpdate(sql.toString(), trxName) == 1) {
			...
			log.log(Level.SEVERE, "Resubmit - Cannot lock " + get_TableName() + "_ID=" + get_ID() + ", Force=" + force
					+ ",RePost=" + repost);
			if (!p_po.isActive())
		...
		if (!p_Status.equals(STATUS_Posted) && !p_Status.equals(STATUS_Deferred)) {
			...
			Text.append(" - ").append(cn.substring(cn.lastIndexOf('.'))).append(" (").append(getDocumentType())
					.append(" - " + Msg.getElement(Env.getCtx(), "DocumentNo") + "=").append(getDocumentNo())
					.append(" - " + Msg.getElement(Env.getCtx(), "DateAcct") + "=")
					.append(dateFormat.format(getDateAcct())).append(" - " + Msg.getMsg(Env.getCtx(), "Amount") + "=")
					.append(numberFormat.format(getAmount()))
					.append(" - " + Msg.getElement(Env.getCtx(), "DocStatus") + "=")
					.append(MRefList.getListName(getCtx(), 131, m_DocStatus))
					.append(" - " + Msg.getMsg(Env.getCtx(), "PeriodOpen") + "=")
					.append(Msg.getMsg(Env.getCtx(), String.valueOf(isPeriodOpen())))
					.append(" - " + Msg.getElement(Env.getCtx(), "IsBalanced") + "=")
					.append(Msg.getMsg(Env.getCtx(), String.valueOf(isBalanced())))
	...
	protected int deleteAcct() {
		StringBuilder sql = new StringBuilder("DELETE FROM Fact_Acct WHERE AD_Table_ID=").append(get_Table_ID())
				.append(" AND Record_ID=").append(p_po.get_ID()).append(" AND C_AcctSchema_ID=")
	...
	private void unlock() {
		...
		if (!m_manageLocalTrx)
			trxName = getTrxName(); // on trx if it's in client
		StringBuilder sql = new StringBuilder("UPDATE ");
		sql.append(get_TableName()).append(" SET Processing='N' WHERE ").append(get_TableName()).append("_ID=")
				.append(p_po.get_ID());
	...
	public void setDocumentType(String DocumentType) {
		...
		// IDEMPIERE-3342 - prefer the category defined for the doctype if there is such
		// column in the table
		...
		if (m_DocumentType == null) {
			log.log(Level.SEVERE,
					"No DocBaseType for C_DocType_ID=" + getC_DocType_ID() + ", DocumentNo=" + getDocumentNo());
		}
		...
		if (m_GL_Category_ID == 0) {
			String sql = "SELECT GL_Category_ID FROM GL_Category " + "WHERE AD_Client_ID=? "
	...
	public boolean isConvertible(MAcctSchema acctSchema) {
		...
		while (it.hasNext() && convertible) {
			...
			if (C_Currency_ID != acctSchema.getC_Currency_ID()) {
				BigDecimal amt = MConversionRate.getRate(C_Currency_ID, acctSchema.getC_Currency_ID(), getDateAcct(),
						getC_ConversionType_ID(), getAD_Client_ID(), getAD_Org_ID());
				if (amt == null) {
					convertible = false;
					log.warning("NOT from C_Currency_ID=" + C_Currency_ID + " to " + acctSchema.getC_Currency_ID()
							+ " - " + toString());
		...
		if (log.isLoggable(Level.FINE))
			log.fine("Convertible=" + convertible + ", AcctSchema C_Currency_ID=" + acctSchema.getC_Currency_ID()
					+ " - " + toString());
		return convertible;
	} // isConvertible

	/**
	 * Calculate Period from DateAcct. m_C_Period_ID is set to -1 of not open to 0
	 * if not found
	 */
	public void setPeriod() {
		...
		if (getPO().get_TableName().equals(MInvoice.Table_Name)
				|| getPO().get_TableName().equals(MPayment.Table_Name)) {
			if (getPO().columnExists("DateAcctCustom")) {
				if (getPO().get_Value("DateAcctCustom") != null)
					setDateAcct((Timestamp) getPO().get_Value("DateAcctCustom"));
			}
		}
	...
	public int getValidCombination_ID(int AcctType, MAcctSchema as) {
		...
		/** Inventory Accounts */
			...
			// "SELECT W_Inventory_Acct, W_Revaluation_Acct, W_InvActualAdjust_Acct FROM
			// M_Warehouse_Acct WHERE M_Warehouse_ID=? AND C_AcctSchema_ID=?";
		...
		// No account
		if (Account_ID == 0) {
			log.warning("NO account Type=" + AcctType + ", Record=" + p_po.get_ID());
			return 0;
		}
		return Account_ID;
	} // getAccount_ID
	...
	public int getC_DocType_ID() {
		...
		if (index != -1) {
			...
			if (p_po.get_TableName().equals(I_M_MatchPO.Table_Name)) {
				int docTypeId = DB.getSQLValue((String) null, DOC_TYPE_BY_DOC_BASE_TYPE_SQL, p_po.getAD_Client_ID(),
