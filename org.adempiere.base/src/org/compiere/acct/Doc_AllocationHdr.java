...
public class Doc_AllocationHdr extends Doc {
	 ...
	 * </pre>
	 * 
	 * Tax needs to be corrected for discount and write-off; Currency gain and loss
	 * is realized here.
	 * 
	 * @param as accounting schema
	 * @return Fact
	...
	public ArrayList<Fact> createFacts(MAcctSchema as) {
		...
		for (int i = 0; i < p_lines.length; i++) {
			...
			if (invoice == null) {
				...
				if (line.getC_Invoice_ID() == 0 && line.getC_Payment_ID() == 0 && line.getC_Charge_ID() != 0) {
					...
					for (FactLine fline : Lines)
						amt = amt.add(fline.getAcctBalance());

					amt = amt.signum() < 0 ? amt.negate() : amt;
					BigDecimal amtsource = line.getAmtSource().signum() < 0 ? line.getAmtSource().negate()
							: line.getAmtSource();
					BigDecimal rate = amt.divide(amtsource, 12, RoundingMode.HALF_UP);
			...
			// Sales Invoice
				...
				if ((!as.isPostIfClearingEqual()) && acct_unallocated_cash != null
					...
					if (line.getC_Payment_ID() != 0) {
						...
						BigDecimal paymentAccounted = getaccountedRate(payment, null, getTrxName(), as,
								getPaymentAcct(as, line.getC_Payment_ID()).getAccount_ID());

						if (paymentAccounted.signum() != 0)
							line.setCurrencyRate(paymentAccounted);
				...
				if (as.isAccrual()) {
					...
					BigDecimal invoiceamt = getaccountedRate(null, invoice, getTrxName(), as, bpAcct.getAccount_ID());
					if (invoiceamt.signum() != 0)
						line.setCurrencyRate(invoiceamt);
			...
			{
				p_Error = createInvoiceGainLoss(line, as, fact, bpAcct, invoice, allocationSource, allocationAccounted);
				if (p_Error != null)
					return null;
			}
		...
		/*
		 * // rounding adjustment if (getC_Currency_ID() != as.getC_Currency_ID()) {
		 * p_Error = createInvoiceRoundingCorrection(as, fact, bpAcct); if (p_Error !=
		 * null) return null; p_Error = createPaymentRoundingCorrection(as, fact); if
		 * (p_Error != null) return null; }
		 */
	...
	private BigDecimal createCashBasedAcct(MAcctSchema as, Fact fact, MInvoice invoice, BigDecimal allocationSource) {
		...
		if (log.isLoggable(Level.CONFIG))
			log.config("Multiplier=" + percent + " - GrandTotal=" + invoice.getGrandTotal() + " - Allocation Source="
					+ allocationSource);

		// Get Invoice Postings
		Doc_Invoice docInvoice = (Doc_Invoice) Doc.get(as, MInvoice.Table_ID, invoice.getC_Invoice_ID(), getTrxName());
		...
		if (as.isCreatePOCommitment() && !invoice.isSOTrx()) {
			...
			for (int i = 0; i < lines.length; i++) {
				Fact factC = Doc_Order.getCommitmentRelease(as, this, lines[i].getQtyInvoiced(),
						lines[i].getC_InvoiceLine_ID(), BigDecimal.valueOf(percent));
				if (factC == null)
					return null;
				m_facts.add(factC);
			}
	...
	private boolean createTaxCorrection(MAcctSchema as, Fact fact, DocLine_Allocation line, MAccount DiscountAccount,
		...
		String sql = "SELECT * " + "FROM Fact_Acct " + "WHERE AD_Table_ID=? AND Record_ID=?" // Invoice
	 ...
	 */
	private String createInvoiceGainLoss(DocLine line, MAcctSchema as, Fact fact, MAccount acct, MInvoice invoice,
		...
		StringBuilder sql = new StringBuilder()
				...
				.append(" FROM Fact_Acct ").append("WHERE AD_Table_ID=? AND Record_ID=?")
				.append(" AND C_AcctSchema_ID=?").append(" AND Account_ID=?").append(" AND PostingType='A'");

		// For Invoice
		List<Object> valuesInv = DB.getSQLValueObjectsEx(getTrxName(), sql.toString(), MInvoice.Table_ID,
				invoice.getC_Invoice_ID(), as.getC_AcctSchema_ID(), acct.getAccount_ID());
		...
		//
		StringBuilder description = new StringBuilder("Invoice=(").append(invoice.getC_Currency_ID()).append(")")
				.append(invoiceSource).append("/").append(invoiceAccounted).append(" - Allocation=(")
				.append(getC_Currency_ID()).append(")").append(allocationSource).append("/")
		...
		{
			int stdPrecision = MCurrency.getStdPrecision(getCtx(), getC_Currency_ID());
			BigDecimal allocationAccounted0 = Env.ZERO;

			if (invoice.isOverrideCurrencyRate()) {
				allocationAccounted0 = invoice.getCurrencyRate().multiply(allocationSource);
				if (allocationAccounted0.scale() > stdPrecision)
					allocationAccounted0 = allocationAccounted0.setScale(stdPrecision, RoundingMode.HALF_UP);
			} else {

				allocationAccounted0 = MConversionRate.convert(getCtx(), allocationSource, getC_Currency_ID(),
	...
	/**************************************************************************
	 * Create Realized Gain & Loss. Compares the Accounted Amount of the Payment to
	 * the Accounted Amount of the Allocation
	 * 
	 * @param line                Allocation line
	 * @param as                  accounting schema
	 ...
	 * @return Error Message or null if OK
	 */
	private String createPaymentGainLoss(DocLine line, MAcctSchema as, Fact fact, MAccount acct, MPayment payment,
			BigDecimal allocationSource, BigDecimal allocationAccounted) {
		...
		// For Payment
		List<Object> valuesPay = DB.getSQLValueObjectsEx(getTrxName(), sql.toString(), MPayment.Table_ID,
				payment.getC_Payment_ID(), as.getC_AcctSchema_ID(), acct.getAccount_ID());
		...
		//
		StringBuilder description = new StringBuilder("Payment=(").append(payment.getC_Currency_ID()).append(")")
				.append(paymentSource).append("/").append(paymentAccounted).append(" - Allocation=(")
				.append(getC_Currency_ID()).append(")").append(allocationSource).append("/")
				.append(allocationAccounted);
		if (log.isLoggable(Level.FINE))
			log.fine(description.toString());
		...
		if (allocationSource.abs().compareTo(paymentSource.abs()) == 0) {
			...
			if (payment.isOverrideCurrencyRate()) {
				allocationAccounted0 = payment.getCurrencyRate().multiply(allocationSource);
				if (allocationAccounted0.scale() > stdPrecision)
					allocationAccounted0 = allocationAccounted0.setScale(stdPrecision, RoundingMode.HALF_UP);
			} else {

				allocationAccounted0 = MConversionRate.convert(getCtx(), allocationSource, getC_Currency_ID(),
						as.getC_Currency_ID(), payment.getDateAcct(), payment.getC_ConversionType_ID(),
		...
		MAccount loss = MAccount.get(as.getCtx(), as.getAcctSchemaDefault().getRealizedLoss_Acct());
		//
		if ((payment.isReceipt() && payment.getPayAmt().signum() >= 0)
				|| (!payment.isReceipt() && payment.getPayAmt().signum() < 0)) {
	...
	/**************************************************************************
	 * Create Rounding Correction. Compares the Accounted Amount of the AR/AP
	 * Invoice to the Accounted Amount of the AR/AP Allocation
	 * 
	 * @param as   accounting schema
	 * @param fact fact
	 * @param acct account
	...
	private String createInvoiceRoundingCorrection(MAcctSchema as, Fact fact, MAccount acct) {
			...
			if (valuesInv != null && valuesInv.size() >= 4) {
				...
				if (invoice.getReversal_ID() == 0 || invoice.get_ID() < invoice.getReversal_ID()) {
					if ((invoice.isSOTrx() && invoice.getGrandTotal().signum() >= 0 && !invoice.isCreditMemo())
							|| (invoice.isSOTrx() && invoice.getGrandTotal().signum() < 0 && invoice.isCreditMemo())
							|| (!invoice.isSOTrx() && invoice.getGrandTotal().signum() >= 0 && invoice.isCreditMemo())
							|| (!invoice.isSOTrx() && invoice.getGrandTotal().signum() < 0
									&& !invoice.isCreditMemo())) {
						invoiceSource = (BigDecimal) valuesInv.get(0); // AmtSourceDr
						invoiceAccounted = (BigDecimal) valuesInv.get(1); // AmtAcctDr
					} else {
						invoiceSource = (BigDecimal) valuesInv.get(2); // AmtSourceCr
		...
		for (FactLine factLine : factlines) {
			if (factLine.getLine_ID() > 0) {
				...
				if (allocationLine.getC_Invoice_ID() > 0) {
					...
					} else if (factLine.getAccount_ID() == gain.getAccount_ID()
							|| factLine.getAccount_ID() == loss.getAccount_ID()) {
		...
		for (MInvoice invoice : invList) {
			...
			for (MAllocationHdr alloc : allocations) {
				...
				StringBuilder sql = new StringBuilder()
						.append("SELECT SUM(AmtSourceDr), SUM(AmtAcctDr), SUM(AmtSourceCr), SUM(AmtAcctCr)")
						.append(" FROM Fact_Acct ").append("WHERE AD_Table_ID=? AND Record_ID=?") // allocation
						.append(" AND C_AcctSchema_ID=?").append(" AND PostingType='A'").append(" AND Account_ID=?")
				...
				List<Object> valuesAlloc = DB.getSQLValueObjectsEx(getTrxName(), sql.toString(),
						MAllocationHdr.Table_ID, alloc.get_ID(), as.getC_AcctSchema_ID(), acct.getAccount_ID(),
				...
				sql = new StringBuilder()
						.append("SELECT SUM(AmtSourceDr), SUM(AmtAcctDr), SUM(AmtSourceCr), SUM(AmtAcctCr)")
						.append(" FROM Fact_Acct ").append("WHERE AD_Table_ID=? AND Record_ID=?") // allocation
						.append(" AND C_AcctSchema_ID=?").append(" AND PostingType='A'")
						.append(" AND (Account_ID=? OR Account_ID=? OR Account_ID=?)")
						.append(" AND Description LIKE 'Invoice%'")
						.append(" AND Line_ID IN (SELECT C_AllocationLine_ID FROM C_AllocationLine WHERE C_AllocationHdr_ID=? AND C_Invoice_ID=?)");

				// For Allocation
				valuesAlloc = DB.getSQLValueObjectsEx(getTrxName(), sql.toString(), MAllocationHdr.Table_ID,
						alloc.get_ID(), as.getC_AcctSchema_ID(), gain.getAccount_ID(), loss.getAccount_ID(),
						as.getCurrencyBalancing_Acct().getAccount_ID(), alloc.get_ID(), invoice.getC_Invoice_ID());
				if (valuesAlloc != null && valuesAlloc.size() >= 4) {
					totalAmtSourceDr = (BigDecimal) valuesAlloc.get(0);
					if (totalAmtSourceDr == null)
						totalAmtSourceDr = Env.ZERO;
					...
					allocateSource = allocateSource.subtract(totalAmtSourceDr).subtract(totalAmtSourceCr)
							.add(currencyAdjustment);
				}
			}
		...
		for (MInvoice invoice : invList) {
			...
			StringBuilder description = new StringBuilder("Invoice=(").append(getC_Currency_ID()).append(")")
					.append(invSource).append("/").append(invAccounted).append(" - Allocation=(")
					.append(getC_Currency_ID()).append(")").append(allocInvSource).append("/")
					.append(allocInvAccounted);
			if (log.isLoggable(Level.FINE))
				log.fine(description.toString());
			...
			if ((invoice.isSOTrx() && invoice.getGrandTotal().signum() >= 0 && !invoice.isCreditMemo())
				...
				if (!fact.isAcctBalanced()) {
					if (as.isCurrencyBalancing() && as.getC_Currency_ID() != invoice.getC_Currency_ID())
						fl = fact.createLine(null, as.getCurrencyBalancing_Acct(), as.getC_Currency_ID(),
	...
	/**************************************************************************
	 * Create Rounding Correction. Compares the Accounted Amount of the Payment to
	 * the Accounted Amount of the Allocation
	 * 
	 * @param as   accounting schema
	 * @param fact fact
	...
	private String createPaymentRoundingCorrection(MAcctSchema as, Fact fact) {
		...
		for (MPayment payment : payList) {
			...
			// For Payment
			List<Object> valuesPay = DB.getSQLValueObjectsEx(getTrxName(), sql.toString(), MPayment.Table_ID,
					payment.getC_Payment_ID(), as.getC_AcctSchema_ID(),
					htPayAcct.get(payment.getC_Payment_ID()).getAccount_ID());
			if (valuesPay != null && valuesPay.size() >= 4) {
		...
		for (MPayment payment : payList) {
			...
			for (MAllocationHdr alloc : allocations) {
				...
				List<Object> valuesAlloc = DB.getSQLValueObjectsEx(getTrxName(), sql.toString(),
						...
						htPayAcct.get(payment.getC_Payment_ID()).getAccount_ID(), alloc.get_ID(),
				...
				valuesAlloc = DB.getSQLValueObjectsEx(getTrxName(), sql.toString(), MAllocationHdr.Table_ID,
						alloc.get_ID(), as.getC_AcctSchema_ID(), gain.getAccount_ID(), loss.getAccount_ID(),
						as.getCurrencyBalancing_Acct().getAccount_ID(), alloc.get_ID(), payment.getC_Payment_ID());
				if (valuesAlloc != null && valuesAlloc.size() >= 4) {
					totalAmtSourceDr = (BigDecimal) valuesAlloc.get(0);
					if (totalAmtSourceDr == null)
						totalAmtSourceDr = Env.ZERO;
		...
		for (MPayment payment : payList) {
			...
			StringBuilder description = new StringBuilder("Payment=(").append(getC_Currency_ID()).append(")")
					.append(paySource).append("/").append(payAccounted).append(" - Allocation=(")
					.append(getC_Currency_ID()).append(")").append(allocPaySource).append("/")
					.append(allocPayAccounted);
			if (log.isLoggable(Level.FINE))
				log.fine(description.toString());
			BigDecimal acctDifference = null;
			...
			if ((payment.isReceipt() && payment.getPayAmt().signum() >= 0)
					|| (!payment.isReceipt() && payment.getPayAmt().signum() < 0)) {
				FactLine fl = fact.createLine(null, htPayAcct.get(payment.getC_Payment_ID()), as.getC_Currency_ID(),
						acctDifference.negate());
				fl.setDescription(description.toString());
				fl.setLine_ID(C_AllocationLine_ID == null ? 0 : C_AllocationLine_ID);
				if (!fact.isAcctBalanced()) {
	...
	private FactLine balanceAccounting(MAcctSchema as, Fact fact) {
		...
		if (!fact.isAcctBalanced()) {
			...
			if (as.isCurrencyBalancing() && acctDifference.abs().compareTo(TOLERANCE) < 0)
				line = fact.createLine(null, as.getCurrencyBalancing_Acct(), as.getC_Currency_ID(),
						acctDifference.negate());
			else
	...
	private BigDecimal getaccountedRate(MPayment payment, MInvoice Invoice, String TrxName, MAcctSchema as,
			int Account_ID) {
		...
		if (payment != null) {
			...
			// For Payment
			List<Object> valuesPay = DB.getSQLValueObjectsEx(TrxName, sql.toString(), MPayment.Table_ID,
					payment.getC_Payment_ID(), as.getC_AcctSchema_ID(), Account_ID);
			if (valuesPay.get(0) != null && valuesPay.get(1) != null && valuesPay.get(2) != null
					&& valuesPay.get(3) != null)
				if (((BigDecimal) valuesPay.get(0)).signum() != 0 && ((BigDecimal) valuesPay.get(1)).signum() != 0)
					amount = ((BigDecimal) valuesPay.get(1)).divide(((BigDecimal) valuesPay.get(0)), 12,
							RoundingMode.HALF_UP);// AmtAcctDr
				else
					amount = ((BigDecimal) valuesPay.get(3)).divide(((BigDecimal) valuesPay.get(2)), 12,
							RoundingMode.HALF_UP);// AmtAcctCr

		} else {

			// For Invoice
			List<Object> valuesInvoice = DB.getSQLValueObjectsEx(TrxName, sql.toString(), MInvoice.Table_ID,
					Invoice.getC_Invoice_ID(), as.getC_AcctSchema_ID(), Account_ID);
			if (valuesInvoice.get(0) != null && valuesInvoice.get(1) != null && valuesInvoice.get(2) != null
					&& valuesInvoice.get(3) != null)
				if (((BigDecimal) valuesInvoice.get(0)).signum() != 0
						&& ((BigDecimal) valuesInvoice.get(1)).signum() != 0)
					amount = ((BigDecimal) valuesInvoice.get(1)).divide(((BigDecimal) valuesInvoice.get(0)), 12,
							RoundingMode.HALF_UP);// AmtAcctDr
				else
					amount = ((BigDecimal) valuesInvoice.get(3)).divide(((BigDecimal) valuesInvoice.get(2)), 12,
							RoundingMode.HALF_UP);// AmtAcctCr
...
class Doc_AllocationTax {
	...
	public boolean createEntries(MAcctSchema as, Fact fact, DocLine line) {
		...
		for (int i = 0; i < m_facts.size(); i++) {
			...
			Doc doc = DocManager.getDocument(as, MInvoice.Table_ID, factAcct.getRecord_ID(),
					line.getPO().get_TrxName());
			MDocType dt = new MDocType(Env.getCtx(), (doc != null) ? doc.getC_DocType_ID() : -1,
					line.getPO().get_TrxName());
			...
			if (m_WriteOffAmt.signum() != 0) {
				...
				if (Env.ZERO.compareTo(factAcct.getAmtSourceDr()) != 0) {
					...
					if (amount.signum() != 0) {
						if (m_IsSOTrx) {
							if (docBaseType.equals(MDocType.DOCBASETYPE_ARCreditMemo)) {
								fact.createLine(line, m_WriteOffAccount, as.getC_Currency_ID(), amount.negate(), null);
								fact.createLine(line, taxAcct, as.getC_Currency_ID(), null, amount.negate());
							} else {
				...
				// Original Tax is CR - need to correct it DR
					...
					if (amount.signum() != 0) {
						if (m_IsSOTrx) {
							if (docBaseType.equals(MDocType.DOCBASETYPE_ARCreditMemo)) {
								fact.createLine(line, taxAcct, as.getC_Currency_ID(), amount.negate(), null);
								fact.createLine(line, m_WriteOffAccount, as.getC_Currency_ID(), null, amount.negate());
							} else {
								fact.createLine(line, taxAcct, as.getC_Currency_ID(), amount, null);
